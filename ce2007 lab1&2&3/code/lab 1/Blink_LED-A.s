...
leds_on		MOVS	R0,#0xFF			; To turn on all 10 LEDs, value need = 0x3FF;  pass the value through R0
		LSLS	R0,R0,#2			; Logical Left shift by 2 (for LEDG9 & LEDG8)
		ADDS	R0,#0x03			; Add 0x03 for the Least Significant LEDs (LEDG1 & LEDG0)
		STR	R0,[R2,#0x00]			; Store the value to the DE0_LED address
		BL	DELAY				; Call the DELAY function
leds_off	MOVS	r0,#0x00			; MOVE new values to r0 to turn off the LED
		STR	r0,[r2,#0x00]			; Store the value to the DE0_LED address
		BL	DELAY				; Call the DELAY function
		B	leds_on				; Repeat
