...
int VpnToPhyPage(int vpn)
{
    ...
    //======================================================
    FILE *f = fopen("debug info", "a");
    IptEntry* iptHhh = hashIPT(vpn, currentThread->pid);
    if (!(iptHhh->next)) {
        fprintf(f, "\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\n");
        fprintf(f, "query_ vpn: %d currentPid: %d \n", vpn, currentThread->pid);
        fprintf(f, "\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\%\n\n\n");
    } else {
        int x = 0;
        fprintf(f, "===============================\n");
        while (iptHhh->next) {
            iptHhh = iptHhh->next;
            fprintf(f, "entry %d_ vPage: %d pid: %d query_ vpn: %d currentPid: %d \n", x++, iptHhh->vPage, iptHhh->pid, vpn, currentThread->pid);  
        }
        fprintf(f, "===============================\n\n\n");
    }
    fclose(f);
    //======================================================
    IptEntry *currentIptEntry = NULL;
    //printf("vpnToPhyPage =============\n");
    while (iptEntryPtr->next) {
        currentIptEntry = iptEntryPtr->next;
        if (currentIptEntry->pid == currentThread->pid
           && currentIptEntry->vPage == vpn) {
            //page found
            //printf("Page found: vpn = %d phyPage = %d\n", vpn, currentIptEntry->phyPage);
            return currentIptEntry->phyPage;
        }
    }
